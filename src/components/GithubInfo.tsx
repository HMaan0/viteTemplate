  ...
  useEffect(() => {
    const main = async () => {
      try {
        ...
        if (!res.ok) {
          // If the response is not OK, set githubInfo to null
          setGithubInfo(null);
          return;
        }
        ...
        // Set the githubInfo state with the JSON data
        setGithubInfo(json);
      } catch (error) {
        // Handle any errors that occur during the fetch
        console.error("Error fetching GitHub user data:", error);
        setGithubInfo(null); // Optionally, set githubInfo to null in case of error
      }
    };
  ...
  return (
    <>
      {githubInfo != null ? (
        ...
        <p className="text-xl font-bold text-center">{username} Not Found</p>
      )}
    </>
  );
};
